...
class Difix(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_vae=4, mv_unet=False, timestep=999):
        ...
        # VAE patching state (for multiprocessing compatibility - no threading locks)
        self._vae_patched = False
        # Note: No threading lock to avoid serialization issues in multiprocessing

        vae = AutoencoderKL.from_pretrained("stabilityai/sd-turbo", subfolder="vae")
        # NOTE: Skip the immediate patching to fix multiprocessing serialization
        # The patching will be done lazily on first use via _apply_vae_patches()
        ...
        if pretrained_path is not None:
            sd = torch.load(pretrained_path, map_location="cpu")
            logger.info(f"Pretrained VAE rank: {sd['rank_vae']}")
    ...
    def _apply_vae_patches(self):
        """
        Apply VAE encoder/decoder patches lazily to fix multiprocessing serialization.
        
        This method is called on first use of the model to apply the custom forward
        methods to the VAE encoder and decoder. This deferred patching approach
        avoids multiprocessing serialization issues that occur when methods are
        bound during __init__.
        
        Note: No threading locks to avoid serialization issues. This assumes
        single-threaded usage per model instance during inference.
        """
        if self._vae_patched:
            return  # Already patched
                
        # Apply the VAE encoder/decoder patches that were skipped in __init__
    ...
    def forward(self, x, timesteps=None, prompt=None, prompt_tokens=None):
        # Apply VAE patches on first use (fixes multiprocessing serialization)
        self._apply_vae_patches()
