...
def read_smpl_constants(folder):
    ...
    smpl_face_code = (
        smpl_vertex_code[smpl_faces[:, 0]]
        + smpl_vertex_code[smpl_faces[:, 1]]
        + smpl_vertex_code[smpl_faces[:, 2]]
    ) / 3.0
    smpl_tetras = (
        np.loadtxt(os.path.join(folder, "tetrahedrons.txt"), dtype=np.int32) - 1
    )
...
def feat_select(feat, select):
    ...
    dim = feat.shape[1] // 2
    idx = torch.tile((1 - select), (1, dim, 1)) * dim + torch.arange(0, dim).unsqueeze(
        0
    ).unsqueeze(2).type_as(select)
...
def cal_sdf_batch(verts, faces, cmaps, vis, points):
    ...
    if verts.shape[1] == 10475:
        ...
        mouth_faces = (
            torch.as_tensor(SMPLX().smplx_mouth_fid)
            .unsqueeze(0)
            .repeat(Bsize, 1, 1)
            .to(faces.device)
        )
    ...
    return (
        pts_sdf.view(Bsize, -1, 1),
        pts_norm.view(Bsize, -1, 3),
        pts_cmap.view(Bsize, -1, 3),
        pts_vis.view(Bsize, -1, 1),
    )
...
def calculate_mIoU(outputs, labels):
    ...
    return (
        thresholded.mean().detach().cpu().numpy()
...
def add_alpha(colors, alpha=0.7):
    ...
    colors_pad = np.pad(
        colors, ((0, 0), (0, 1)), mode="constant", constant_values=alpha
    )
...
class SMPLX:
    def __init__(self):
        ...
        self.smplx_to_smplx_path = osp.join(
            self.current_dir, "smpl_data/smplx_to_smpl.pkl"
        )

        self.smplx_eyeball_fid = osp.join(self.current_dir, "smpl_data/eyeball_fid.npy")
        self.smplx_fill_mouth_fid = osp.join(
            self.current_dir, "smpl_data/fill_mouth_fid.npy"
        )
