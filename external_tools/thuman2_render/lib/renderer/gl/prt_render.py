...
class PRTRender(CamRender):
    ...
    def cleanup(self):
        ...
        # Use a temporary list of keys if modifying the dictionary during iteration
        keys_to_delete = list(self.vert_buffer.keys())
        for key in keys_to_delete:
            # Delete VBOs
            if key in self.vert_buffer: glDeleteBuffers(1, [self.vert_buffer.pop(key)])
            if key in self.norm_buffer: glDeleteBuffers(1, [self.norm_buffer.pop(key)])
            if key in self.uv_buffer: glDeleteBuffers(1, [self.uv_buffer.pop(key)])
            if key in self.vert_label_buffer: glDeleteBuffers(1, [self.vert_label_buffer.pop(key)])
            if key in self.tan_buffer: glDeleteBuffers(1, [self.tan_buffer.pop(key)])
            if key in self.btan_buffer: glDeleteBuffers(1, [self.btan_buffer.pop(key)])
            if key in self.prt1_buffer: glDeleteBuffers(1, [self.prt1_buffer.pop(key)])
            if key in self.prt2_buffer: glDeleteBuffers(1, [self.prt2_buffer.pop(key)])
            if key in self.prt3_buffer: glDeleteBuffers(1, [self.prt3_buffer.pop(key)])

            # Delete Textures specific to PRTRender
            if key in self.render_texture_mat:
                texture_smplrs_to_delete = list(self.render_texture_mat[key].keys())
                for smplr in texture_smplrs_to_delete:
                    glDeleteTextures([self.render_texture_mat[key].pop(smplr)])
                if not self.render_texture_mat[key]: # if dict for key is empty
                    del self.render_texture_mat[key]
        
        # Clear data dictionaries
        self.vert_data.clear()
        self.vert_label_data.clear()
        self.norm_data.clear()
        self.tan_data.clear()
        self.btan_data.clear()
        self.prt1_data.clear()
        self.prt2_data.clear()
        self.prt3_data.clear()
        self.uv_data.clear()
        
        # If render_texture_mat might not have all keys that buffers did, or vice-versa,
        # ensure it's fully cleared if it's not empty after the loop.
        # However, the loop structure should handle it. For safety:
        self.render_texture_mat.clear()


        self.vertex_dim.clear()
        self.n_vertices.clear()
        self.label_dim.clear()
