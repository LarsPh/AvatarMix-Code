...
model_init_params = dict(
    gender="male",
    model_type="smplx",
    model_path=str(Path(os.environ.get("AVATARMIX_ASSET_ROOT", str(Path(__file__).resolve().parents[4] / "external_assets"))) / "smpl"),
...
def load_fit_body(
    fitted_path, scale, smpl_type="smplx", smpl_gender="neutral", noise_dict=None
):
    ...
    smpl_verts = (
        (smpl_out.vertices[0] * param["scale"] + param["translation"]) * scale
    ).detach()
    smpl_joints = (
        (smpl_out.joints[0] * param["scale"] + param["translation"]) * scale
    ).detach()
    smpl_mesh = trimesh.Trimesh(
        smpl_verts, smpl_model.faces, process=False, maintain_order=True
    )
