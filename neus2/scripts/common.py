...
def require_pyngp():
	"""Import the CMake-built pybind11 extension `pyngp`.

	This repo does not build/install `pyngp` via `pyproject.toml`. It is produced by CMake
	(target `pyngp`) and must be built for the *same* Python interpreter that runs scripts.
	"""
	try:
		import pyngp as ngp  # noqa: F401
	except ModuleNotFoundError as e:
		msg = (
			"Could not import 'pyngp'. This is a CMake-built CUDA/pybind11 extension.\n\n"
			"Fix: build it against the current Python interpreter, then rerun.\n\n"
			"From the repo root (neus2/):\n"
			"  PY=$(python -c 'import sys; print(sys.executable)')\n"
			"  cmake -S . -B build -DNGP_BUILD_WITH_GUI=OFF -DNGP_BUILD_WITH_VULKAN=OFF -DNGP_BUILD_WITH_OPTIX=OFF -DPython_EXECUTABLE=\"$PY\"\n"
			"  cmake --build build -j\n\n"
			"If you use uv, run python via uv so PY points to the venv:\n"
