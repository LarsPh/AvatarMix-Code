...
def parse_args():
	...
	parser.add_argument("--save_every_n_steps", type=int, default=10000, help="Save this snapshot after training every n steps. recommended extension: .msgpack")
	...
	if args.dynamic_test:
		...
		while True:
			...
			if args.dynamic_save_mesh:
				...
				save_mesh_path = os.path.join(args.output_path,"evaluation","mesh",f"scene_{os.path.split(args.scene)[-1].replace('.json', '')}_frame_{testbed.current_training_time_frame:04}.obj")
		...
		if n_steps > 0:
			...
			while testbed.frame():
				...
				at_save_step = testbed.training_step % args.save_every_n_steps == 0 
				save_first_frame = (testbed.current_training_time_frame == 0 and testbed.training_step == testbed.first_frame_max_training_step) 
				save_next_frame = (testbed.current_training_time_frame != 0 and testbed.training_step == testbed.next_frame_max_training_step)
				if at_save_step or save_first_frame or save_next_frame:
