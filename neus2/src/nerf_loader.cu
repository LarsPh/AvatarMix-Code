...
NerfDataset load_nerf(const std::vector<filesystem::path>& jsonpaths, float sharpen_amount) {
	...
	for (size_t i = 0; i < jsons.size(); ++i) {
		...
		// Camera distortion
		{
			...
			// This is fine because cx, cy are for all cameras
			if (json.contains("cx")) {
			...
			// This is fine, distoriton is the same for all cameras
		...
		if (json.contains("frames") && json["frames"].is_array()) pool.parallelForAsync<size_t>(0, json["frames"].size(), [&progress, &n_loaded, &result, &images, &json, basepath, image_idx, info, rolling_shutter, principal_point, camera_distortion, part_after_underscore, fix_premult, enable_depth_loading, enable_ray_loading](size_t i) {
			...
			// try to read w, h from frame, if not, use json["w"], json["h"]
			int w = frame.contains("w") ? frame["w"] : json["w"];
			int h = frame.contains("h") ? frame["h"] : json["h"];
			...
			if (x_fl != 0) {
				...
				tlog::info() << "fl_x, fl_y: cam" << i_img << " focal_length: " << result.metadata[i_img].focal_length.x() << " " << result.metadata[i_img].focal_length.y();
			} else if (y_fl != 0) {
				...
				if (frame.contains("intrinsic_matrix")){
					...
					// !This is the bug, we should NOT assume the same w, h for all cameras
					...
					result.metadata[i_img].principal_point.x() = float(intrinsic[0][2])/(float)w;
					result.metadata[i_img].principal_point.y() = float(intrinsic[1][2])/(float)h;
					tlog::info() << "intri: cam" << i_img << " w: " << w << " h: " << h;
					tlog::info() << "intri: cam" << i_img << " focal_length: " << result.metadata[i_img].focal_length.x() << " " << result.metadata[i_img].focal_length.y();
					tlog::info() << "intri: cam" << i_img << " principal_point: " << result.metadata[i_img].principal_point.x() << " " << result.metadata[i_img].principal_point.y();
				}
				else{
			...
			if (frame.contains("cx")) {
				result.metadata[i_img].principal_point.x() = float(frame["cx"]) / (float)(w);
				tlog::info() << "cx: cam" << i_img << " principal_point: " << result.metadata[i_img].principal_point.x() << " " << result.metadata[i_img].principal_point.y();
			}

			if (frame.contains("cy")) {
				result.metadata[i_img].principal_point.y() = float(frame["cy"]) / (float)(h);
				tlog::info() << "cy: cam" << i_img << " principal_point: " << result.metadata[i_img].principal_point.x() << " " << result.metadata[i_img].principal_point.y();
			}
