...
def make_frameset_data(dataset_config, split='train', frm_list=None):
    ...
    elif dataset_config.frameset_type == 'avatarrex':
        from .avatarrex_reader import AvatarRexDataset
        frameset = AvatarRexDataset(dataset_config, split=split)
    elif dataset_config.frameset_type == 'thuman2':
        from .thuman2_reader import THuman2Dataset
        frameset = THuman2Dataset(dataset_config, split=split)
    else:
...
def make_dataloader(frameset, shuffle=False, batch_size=1, 
    ...
    if sampler is not None:
        shuffle = False
    dataloader = torch.utils.data.DataLoader(frameset, shuffle=shuffle, sampler=sampler,
