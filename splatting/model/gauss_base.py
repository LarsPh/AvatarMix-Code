...
class GaussianBase(nn.Module):
    ...
    # --- Default property interface expected by gaussian_renderer ---
    # Subclasses (e.g., SplattingAvatarModel) may override these to add mesh/deformation logic.
    @property
    def get_xyz(self):
        return self._xyz
    ...
    def get_features(self):
        # Concatenate DC and remaining SH features along channel dimension to match renderer expectation
    ...
    def get_rotation_embed(self):
        # Default: identity rotation per Gaussian (w, x, y, z) = (1, 0, 0, 0)
        if hasattr(self, '_rotation') and self._rotation is not None:
            return torch.zeros_like(self._rotation).scatter_(1, torch.zeros(self._rotation.shape[0], 1, dtype=torch.long, device=self._rotation.device), 1.0)
        # Fallback: no rotations yet, return a single identity quaternion
        return torch.tensor([[1.0, 0.0, 0.0, 0.0]], dtype=torch.float32, device='cuda')
    ...
    def render_to_camera(self, viewpoint_cam, pipe, background='random', scaling_modifer=1.0):
        ...
        # Expose the per-iteration background so downstream code can composite multiple
        # GT variants consistently (e.g., hybrid supervision: original vs refined GT).
        out.update({'background': background})

        if hasattr(viewpoint_cam, 'original_image'):
            # Some cameras (e.g., synthetic orbit cameras) may not have a valid alpha mask.
            # Treat an empty mask as "no mask" and avoid mixing with background.
            gt_image = viewpoint_cam.original_image.cuda()
            use_mask = hasattr(viewpoint_cam, 'gt_alpha_mask') and \
                       isinstance(viewpoint_cam.gt_alpha_mask, torch.Tensor) and \
                       viewpoint_cam.gt_alpha_mask.numel() > 0
