    ...
    const float3 &v2)
{
  ...
  // pytorch3d current implementation
  float3 p0 = v1 - v0;
  float3 p1 = v2 - v0;
  float3 p2 = p - v0;
...
__device__ inline bool IsInsideTriangle(
    ...
    const float3 &v1,
    const float3 &v2,
    const double min_triangle_area = 5e-3)
{
  bool inside = false;
    ...
    const float3 &v2)
{
  ...
  if ((is_inside) && (norm_normal > kEpsilon))
  {
    ...
    //     dist = t * t;
    dist = min(norm(p - v0), norm(p - v1));
    dist = min(dist, norm(p - v2));
  }
  else
  {
...
PointLine3DistanceCoordsForward(const float3 &p, const float3 &v0, const float3 &v1)
{
  const float3 v1v0 = v1 - v0;
  const float3 pv0 = p - v0;
  const float t_bot = dot(v1v0, v1v0);
  ...
  const float3 diff = p - p_proj;
  const float dist = dot(diff, diff);
  return make_float2(dist, tt);
}

__device__ float closest_pt_on_line(const float3 &a, const float3 &b, const float3 &c, float3 &closest_pt)
{
  float3 ac = c - a;
  float3 ab = b - a;
  float t = dot(ac, ab) / dot(ab, ab);
  // t = fminf(0.f, t);
  // t = fmaxf(t, 1.f); // !BUG: This is NOT clamping, but picking up min/max -> t is always 1
  if (dot(ab, ab) < kEpsilon)
  {
    t = 0.f;
  }
  t = __saturatef(t); // clamp t to [0,1]
  closest_pt = a + t * ab;
  const float3 diff = closest_pt - c;
  const float dist = dot(diff, diff);
  return dist;
  // return norm(closest_pt - c);
}

__device__ bool pt_in_triangle(const float3 &v0, const float3 &v1, const float3 &v2, const float3 &p)
{
  float3 a = v0 - p;
  float3 b = v1 - p;
  float3 c = v2 - p;
...
// PointFaceDistance with barycentric coordinates
// No Backward is implemented
__device__ inline thrust::tuple<float, float3>
PointTriangle3DistanceCoordsForward(
    const float3 &p,
    const float3 &v0,
    const float3 &v1,
    const float3 &v2,
    const float min_triangle_area = 5e-3)
{
  ...
  {
    float3 closest_pt_tmp;
    dist = closest_pt_on_line(v0, v1, p, closest_pt_tmp);
    closest_pt = closest_pt_tmp;
    float d2 = closest_pt_on_line(v1, v2, p, closest_pt_tmp);
    if (d2 < dist)
    {
      closest_pt = closest_pt_tmp;
      dist = d2;
    }
    float d3 = closest_pt_on_line(v2, v0, p, closest_pt_tmp);
    if (d3 < dist)
    {
      closest_pt = closest_pt_tmp;
      dist = d3;
  ...
  float3 bary = BarycentricCoords3Forward(closest_pt, v0, v1, v2);

  return thrust::make_tuple(dist, bary);
}