...
class SMPL(nn.Module):
        ...
        **kwargs
        ...
        if create_transl:
            if transl is None:
                ...
                default_transl = transl.to(dtype=dtype)
            else:
    ...
    def get_root_T(self, global_orient=None, transl=None, orient_origin=None) -> torch.Tensor:
        ...
        if create_left_hand_pose:
            if left_hand_pose is None:
                default_lhand_pose = torch.zeros([batch_size, hand_pose_dim],
                                                 dtype=dtype)
            elif isinstance(left_hand_pose, torch.Tensor):
                default_lhand_pose = left_hand_pose.to(dtype=dtype)
            else:
        ...
        if create_right_hand_pose:
            if right_hand_pose is None:
                default_rhand_pose = torch.zeros([batch_size, hand_pose_dim],
                                                 dtype=dtype)
            elif isinstance(right_hand_pose, torch.Tensor):
                default_rhand_pose = right_hand_pose.to(dtype=dtype)
            else:
...
class SMPLX(SMPLH):
    ...
    def __init__(
        ...
        **kwargs
        ...
        data_struct = Struct(**model_data)
        self.tc_faces = data_struct.ft if hasattr(data_struct, 'ft') else None
        self.tc = data_struct.vt if hasattr(data_struct, 'vt') else None

        if hasattr(data_struct, 'vertex_ids'):
            vertex_ids = data_struct.vertex_ids.item()
        else:
        ...
        if hasattr(data_struct, 'lmk_faces_idx'):
        ...
        if create_jaw_pose:
            ...
            elif isinstance(jaw_pose, torch.Tensor):
                default_jaw_pose = jaw_pose.to(dtype=dtype)
            else:
        ...
        if create_leye_pose:
            ...
            elif isinstance(leye_pose, torch.Tensor):
                default_leye_pose = leye_pose.to(dtype=dtype)
            else:
        ...
        if create_reye_pose:
            ...
            elif isinstance(reye_pose, torch.Tensor):
                default_reye_pose = reye_pose.to(dtype=dtype)
            else:
        ...
        if create_expression:
            if expression is None:
                ...
                default_expression = expression.to(dtype=dtype)
            else:
    ...
    def create_mean_pose(self, data_struct, flat_hand_mean=False):
        ...
        pose_mean = np.concatenate([global_orient_mean, body_pose_mean,
                                    ...
                                    self.left_hand_mean.view(-1), self.right_hand_mean.view(-1)],
                                    axis=0)
    ...
    def forward(
        ...
        **kwargs
        ...
        scale_ = int(batch_size / betas.shape[0])
        if scale_ > 1:
            betas = betas.expand(scale_, -1)
        shape_components = torch.cat([betas, expression], dim=-1)

        shapedirs = torch.cat([self.shapedirs, self.expr_dirs], dim=-1)

        ret = lbs(shape_components, full_pose, self.v_template,
        ...
        # first scale, then translation
        use_scale = global_scale is not None
        if global_scale is not None:
            joints = joints * global_scale.unsqueeze(dim=1)
            vertices = vertices * global_scale.unsqueeze(dim=1)

        if apply_trans:
            joints += transl.unsqueeze(dim=1)
            vertices += transl.unsqueeze(dim=1)

        v_shaped = ret['v_shaped']
        v_posed = ret['v_posed']
        # if return_shaped:
        #     v_shaped = self.v_template + blend_shapes(betas, self.shapedirs)
        # else:
        ...
        output = SMPLXOutput(vertices=vertices if return_verts else None,
                             ...
                             v_posed=v_posed,
                             full_pose=full_pose if return_full_pose else None,
                             T=T, T_weighted=T_weighted, weights=W, use_scale=use_scale)
        return output
...
def create(
    ...
    **kwargs
    ...
    elif model_type.lower() == 'smplx' or model_type.lower() == 'smplx-lite':
