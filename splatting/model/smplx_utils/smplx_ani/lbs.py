...
def lbs(
    ...
    if v_shape is None:
        v_shape = torch.zeros_like(v_template)
    if v_pose is None:
        v_pose = torch.zeros_like(v_template)

    v_shaped = v_template + blend_shapes(betas, shapedirs) + v_shape
    ...
    v_posed = pose_offsets + v_shaped + v_pose
