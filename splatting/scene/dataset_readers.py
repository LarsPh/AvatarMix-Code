...
def framesetToCameraInfo(idx, cam, image, image_path=''):
    # Check if the camera object has pre-computed W2C extrinsics and intrinsics
    # This path is used by AvatarRexDataset which loads W2C directly
    if hasattr(cam, 'R_w2c_direct') and hasattr(cam, 'T_w2c_direct'):
        # Use direct W2C values, but transpose R for the expected format
        R = cam.R_w2c_direct.T # Transpose R_w2c here
        T = cam.T_w2c_direct # T is already W2C
        fx = cam.fx
        fy = cam.fy
        cx = cam.cx
        cy = cam.cy
    else:
        # Original logic for datasets providing C2W (cam.R) and world center (cam.c)
        R_c2w = cam.R
        c_world = cam.c[:, None]
        
        # Re-calculate W2C matrix
        R_w2c_calc = R_c2w.T 
        T_w2c_calc = -R_w2c_calc @ c_world
        w2c = np.identity(4)
        w2c[:3, :3] = R_w2c_calc
        w2c[:3, 3] = T_w2c_calc.flatten()

        # Extract R and T in the original way, which includes the transpose for R
        R = np.transpose(w2c)[:3, :3] # This results in R_w2c_calc.T
        T = w2c[:3, 3] # This is T_w2c_calc
