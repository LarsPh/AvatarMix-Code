...
def erode_mask(mask: torch.Tensor, ksize: int, iters: int = 1) -> torch.Tensor:
    """Erode a binary mask using max-pooling on the inverse mask.

    - mask: [1,H,W] or [H,W] with values in {0,1} (float or bool).
    - ksize: odd kernel size >= 1.
    - iters: number of erosion iterations.
    Returns a mask of the same shape (float in [0,1]).
    """
    ...
    x = 1.0 - m  # inverse for erosion via dilation
    k = int(ksize)
    if k % 2 == 0:
        k += 1
    pad = k // 2
    for _ in range(int(iters)):
        x = thf.max_pool2d(x.unsqueeze(0), kernel_size=k, stride=1, padding=pad).squeeze(0)
    out = 1.0 - x
    return out.float()
...
def gaussian_blur_2d(
    ...
    *,
    ...
    """Apply a separable Gaussian blur to BCHW tensor `x`.
    ...
    w = torch.exp(-(t * t) / (2.0 * (sig * sig)))
    w = w / w.sum().clamp_min(1e-12)

    # Separable: first horizontal then vertical (grouped conv, per-channel).
    B, C, H, W = x.shape
    w_h = w.view(1, 1, 1, k).repeat(C, 1, 1, 1)  # [C,1,1,k]
    w_v = w.view(1, 1, k, 1).repeat(C, 1, 1, 1)  # [C,1,k,1]

    y = thf.conv2d(x, w_h, padding=(0, radius), groups=C)
    y = thf.conv2d(y, w_v, padding=(radius, 0), groups=C)
    return y
