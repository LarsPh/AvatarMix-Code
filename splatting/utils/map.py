...
def triangle2projection(triangles):
    ...
    # Debugging: Check determinants
    # Calculate determinants for the 3x3 rotation part first, as that's often the source of degeneracy
    # or the full 4x4. For a rigid transformation, det(R) should be 1.
    # The 4x4 matrix I should have det(R) as its determinant if T doesn't cause issues with affine part.
    # For homogeneous transformation matrices, the determinant of the upper-left 3x3 (rotation/scaling) is key.
    if DEBUG_PRINTING and I.shape[0] == 1: # Assuming batch size of 1 for `triangles` input to this func
        mats_3x3 = I[0, :, 0:3, 0:3] # Shape: [num_triangles, 3, 3]
        dets_3x3 = torch.linalg.det(mats_3x3)
        problematic_indices_3x3 = torch.where(torch.abs(dets_3x3) < 1e-6)[0]
        if problematic_indices_3x3.numel() > 0:
            print(f"[DEBUG] Degenerate 3x3 submatrices (potential R) in triangle2projection found at triangle indices: {problematic_indices_3x3.tolist()}")
            print(f"[DEBUG] Corresponding determinants: {dets_3x3[problematic_indices_3x3].tolist()}")
        else:
            print(f"[DEBUG] No degenerate 3x3 submatrices (potential R) in triangle2projection found")
        # For the full 4x4 matrix, if it's a proper rigid transformation (rotation + translation)
        # the determinant should be det(R). If scaling is involved, it would be det(S*R) = det(S)*det(R).
        # If R is purely rotation, det(R) = 1.
        # A non-invertible homogeneous matrix will have a determinant of 0.
        dets_4x4 = torch.linalg.det(I[0]) # Shape: [num_triangles]
        problematic_indices_4x4 = torch.where(torch.abs(dets_4x4) < 1e-6)[0]
        if problematic_indices_4x4.numel() > 0:
            print(f"[DEBUG] Non-invertible 4x4 matrices (cano_Rt) in triangle2projection found at triangle indices: {problematic_indices_4x4.tolist()}")
            print(f"[DEBUG] Corresponding 4x4 determinants: {dets_4x4[problematic_indices_4x4].tolist()}")
            # You might want to inspect the triangles themselves:
            # problematic_triangles_data = triangles[0, problematic_indices_4x4]
            # print(f"[DEBUG] Problematic triangles data:\n{problematic_triangles_data}")
        else:
            print(f"[DEBUG] No non-invertible 4x4 matrices (cano_Rt) in triangle2projection found")
